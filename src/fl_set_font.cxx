...
static Fl_Fontdesc* built_in_fl_fonts = 0;
...
void fl_free_fonts()
{
  ...
  if (table_size) {
    // Release allocated table and clean up built-in one
    free(fl_fonts);
    fl_fonts = built_in_fl_fonts;
    for (i = FL_FREE_FONT;  i--;  ) {
      Fl_Fontdesc* s = fl_fonts+i;
      s->fontname[0] = 0;
      s->name = 0;
#if !defined(WIN32) && !defined(__APPLE__)
